...
import { bundleBonusUnits, bundleTotalUnits, bundleUnits } from "@/lib/credits/copy";
...
export function PackagePointsSummary({
  ...
  return (
    <div className={compact ? "mt-0.5 space-y-0.5" : "mt-1 space-y-1"}>
      <p className={mutedClass}>{bundleUnits(pkg.credits)}</p>
      <p className={totalClass}>Totaal: {bundleTotalUnits(pkg.credits, pkg.bonus)}</p>
    </div>
  );
}
